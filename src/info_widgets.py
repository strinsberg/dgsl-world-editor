...
class InfoCheck(InfoWidget):
    ...
    def makeWidgets(self):
        InfoWidget.makeWidgets(self)
        tk.Checkbutton(self, variable=self.state_var).grid(row=0,
                column=1)
...
class InfoOption(InfoWidget):
    ...
    def makeWidgets(self):
        InfoWidget.makeWidgets(self)
        tk.OptionMenu(*([self, self.text_var] + self.options)).grid(
                row=0, column=1)
...
class InfoLabel(InfoWidget):
    ...
    def makeWidgets(self):
        InfoWidget.makeWidgets(self)
        tk.Label(self, textvariable=self.text_var).grid(row=0,
                column=1)
...
class InfoSelector(InfoLabel):
    ...
    def select(self):
        obj = self.commands['select'].execute(self.kind,
                self.obj_info['id'])
        if obj:
